...
def handle_command_start(message):
    ...
    logger.info(f"Received /start command from user: {user_display_name}")
    ...
    try:
        ...
        if any(keyword in user_raw_text for keyword in ['photo', 'pic', 'tasvir', 'bhejo', 'dikhao', 'selfie', 'sexy', 'bold', 'hot', 'dark', 'rat', 'night']):
            ...
            if any(k in user_raw_text for k in ['dark', 'rat', 'night', 'dim', 'room', 'light off']):
                aesthetic_style = "dark room ambient lighting, night vibe aesthetic, smartphone flash photography"
            elif any(k in user_raw_text for k in ['sexy', 'bold', 'hot', 'glam', 'attractive']):
                aesthetic_style = "glamorous, bold aesthetic portrait, high-end studio lighting"
        ...
        logger.error(f"Critical execution error inside router: {router_exception}")
...
def server_uptime_health_check():
    return "Virtual Girlfriend Bot is Online & Running Free (Credit-Free Mode)!", 200
